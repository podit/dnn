'''
    GPU debug control script, with a scaled down model
'''
...
def cupify(im, lab, samples): 
    imVec = cp.zeros((samples, im[0].shape[0]), dtype=cp.float32)
    labVec = cp.zeros((samples, lab[0].shape[0]), dtype=cp.float32)
